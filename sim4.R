...
sim = function(alpha){
  SW1E = pWilcoxtest1E = rankSumBF1E = matrix(2,nrow=s,ncol=length(n))
  for(i in 1:length(n)){
    for(j in 1:s){
      if(alpha!=1){ # if alpha=1, no pretest, move on with t.test and JZS BF test
          ...
          pWilcoxtest1E[j,i]=wilcox.test(xDat,yDat,conf.level = 0.95)$p.value
          rankSumBF1E[j,i]=giveBayesFactorZeroOne(rankSumGibbsSampler(xVals=xDat,yVals=yDat,nSamples = 2500,progBar = FALSE))
      } else{
        xDat=rexp(n[i],1)
        yDat=rexp(n[i],1)
        pWilcoxtest1E[j,i]=wilcox.test(xDat,yDat,conf.level = 0.95)$p.value
  ...
  result = list(pWilcoxtest1E,rankSumBF1E)
  result
}
...
# )
system.time(
  results4 <- mclapply(alpha_pre, sim, mc.cores = numCores)
)


# t-test p-values and JZS BFs
alphaWilcoxAlphaPre1E=alphaRankSumBFAlphaPre1E=numeric(length(n)) # p-values & BFs for alpha_pre = 0.100
alphaWilcoxAlphaPre05E=alphaRankSumBFAlphaPre05E=numeric(length(n)) # p-values & BFs for alpha_pre = 0.05
alphaWilcoxAlphaPre01E=alphaRankSumBFAlphaPre01E=numeric(length(n)) # p-values & BFs for alpha_pre = 0.01
alphaWilcoxAlphaPreNoPreE=alphaRankSumBFAlphaPreNoPreE=numeric(length(n)) # p-values & BFs for alpha_pre = 1 ( no pretest)

# Wilcox p-values and rankSumGibbsSampler BFs p-values
# alpha = 0.100
for(j in 1:length(n)){
      alphaWilcoxAlphaPre1E[j]=as.numeric(tbl_df(results4[[1]][[1]][,j]) %>% filter(value < 0.05) %>% summarise(length(value))/s)
}
for(j in 1:length(n)){
      alphaRankSumBFAlphaPre1E[j]=as.numeric(tbl_df(results4[[1]][[2]][,j]) %>% filter(value >= 3) %>% summarise(length(value))/s)
}

# alpha = 0.05
for(j in 1:length(n)){
      alphaWilcoxAlphaPre05E[j]=as.numeric(tbl_df(results4[[2]][[1]][,j]) %>% filter(value < 0.05) %>% summarise(length(value))/s)
}
for(j in 1:length(n)){
      alphaRankSumBFAlphaPre05E[j]=as.numeric(tbl_df(results4[[2]][[2]][,j]) %>% filter(value >= 3) %>% summarise(length(value))/s)
}

# alpha = 0.01
for(j in 1:length(n)){
      alphaWilcoxAlphaPre01E[j]=as.numeric(tbl_df(results4[[3]][[1]][,j]) %>% filter(value < 0.05) %>% summarise(length(value))/s)
}
for(j in 1:length(n)){
      alphaRankSumBFAlphaPre01E[j]=as.numeric(tbl_df(results4[[3]][[2]][,j]) %>% filter(value >= 3) %>% summarise(length(value))/s)
}

# no pretest
for(j in 1:length(n)){
      alphaWilcoxAlphaPreNoPreE[j]=as.numeric(tbl_df(results4[[4]][[1]][,j]) %>% filter(value < 0.05) %>% summarise(length(value))/s)
}
for(j in 1:length(n)){
      alphaRankSumBFAlphaPreNoPreE[j]=as.numeric(tbl_df(results4[[4]][[2]][,j]) %>% filter(value >= 3) %>% summarise(length(value))/s)
}

print("SIM4")
print("alphaWilcoxAlphaPre1E\n")
alphaWilcoxAlphaPre1E
print("alphaRankSumBFAlphaPre1E\n")
alphaRankSumBFAlphaPre1E
print("alphaWilcoxAlphaPre05E\n")
alphaWilcoxAlphaPre05E
print("alphaRankSumBFAlphaPre05E\n")
alphaRankSumBFAlphaPre05E
print("alphaWilcoxAlphaPre01E\n")
alphaWilcoxAlphaPre01E
print("alphaRankSumBFAlphaPre01E\n")
alphaRankSumBFAlphaPre01E
print("alphaWilcoxAlphaPreNoPreE\n")
alphaWilcoxAlphaPreNoPreE
print("alphaRankSumBFAlphaPreNoPreE\n")
alphaRankSumBFAlphaPreNoPreE
