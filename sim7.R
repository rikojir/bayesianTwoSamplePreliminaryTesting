    ...
    for(j in 1:s){
      if(alpha!=1){ # if alpha=1, no pretest, move on with t.test and JZS BF test
        ...
        if(SW1N[j,i]<alpha){
          ...
          BF1N[j,i]=giveBayesFactorZeroOne(rankSumGibbsSampler(xVals=xDat,yVals=yDat,nSamples = 2500,progBar = FALSE))
        }
      } else{
        xDat=rnorm(n[i],0,1)
        yDat=rnorm(n[i],0,1)
...
# )
system.time(
  resultsTwoStageProcedure <- mclapply(alpha_pre, sim, mc.cores = numCores) # parallel execution for all alpha_pre levels
)
print("SIM7")
#p-values and BFs
alphaTwoStageAlphaPre1N=alphaTwoStageAlphaPre1NBF=numeric(length(n)) # p-values & BFs for alpha_pre = 0.100
alphaTwoStageAlphaAlphaPre05N=alphaTwoStageAlphaAlphaPre05NBF=numeric(length(n)) # p-values & BFs for alpha_pre = 0.05
alphaTwoStageAlphaAlphaPre01N=alphaTwoStageAlphaAlphaPre01NBF=numeric(length(n)) # p-values & BFs for alpha_pre = 0.01
alphaTwoStageAlphaAlphaPreNoPreN=alphaTwoStageAlphaAlphaPreNoPreNBF=numeric(length(n)) # p-values & BFs for alpha_pre = 1 ( no pretest)

# alpha = 0.1
for(j in 1:length(n)){
      alphaTwoStageAlphaPre1N[j]=as.numeric(tbl_df(resultsTwoStageProcedure[[1]][[1]][,j]) %>% filter(value < 0.05) %>% summarise(length(value))/s)
}
for(j in 1:length(n)){
      alphaTwoStageAlphaPre1NBF[j]=as.numeric(tbl_df(resultsTwoStageProcedure[[1]][[2]][,j]) %>% filter(value >= 3) %>% summarise(length(value))/s)
}

# alpha = 0.05
for(j in 1:length(n)){
      alphaTwoStageAlphaAlphaPre05N[j]=as.numeric(tbl_df(resultsTwoStageProcedure[[2]][[1]][,j]) %>% filter(value < 0.05) %>% summarise(length(value))/s)
}
for(j in 1:length(n)){
      alphaTwoStageAlphaAlphaPre05NBF[j]=as.numeric(tbl_df(resultsTwoStageProcedure[[2]][[2]][,j]) %>% filter(value >= 3) %>% summarise(length(value))/s)
}

# alpha = 0.01
for(j in 1:length(n)){
      alphaTwoStageAlphaAlphaPre01N[j]=as.numeric(tbl_df(resultsTwoStageProcedure[[3]][[1]][,j]) %>% filter(value < 0.05) %>% summarise(length(value))/s)
}
for(j in 1:length(n)){
      alphaTwoStageAlphaAlphaPre01NBF[j]=as.numeric(tbl_df(resultsTwoStageProcedure[[3]][[2]][,j]) %>% filter(value >= 3) %>% summarise(length(value))/s)
}

# no pretest
for(j in 1:length(n)){
      alphaTwoStageAlphaAlphaPreNoPreN[j]=as.numeric(tbl_df(resultsTwoStageProcedure[[4]][[1]][,j]) %>% filter(value < 0.05) %>% summarise(length(value))/s)
}
for(j in 1:length(n)){
      alphaTwoStageAlphaAlphaPreNoPreNBF[j]=as.numeric(tbl_df(resultsTwoStageProcedure[[4]][[2]][,j]) %>% filter(value >= 3) %>% summarise(length(value))/s)
}

print("SIM7")
print("alphaTwoStageAlphaPre1N\n")
alphaTwoStageAlphaPre1N
print("alphaTwoStageAlphaPre1NBF\n")
alphaTwoStageAlphaPre1NBF
print("alphaTwoStageAlphaAlphaPre05N\n")
alphaTwoStageAlphaAlphaPre05N
print("alphaTwoStageAlphaAlphaPre05NBF\n")
alphaTwoStageAlphaAlphaPre05NBF
print("alphaTwoStageAlphaAlphaPre01N\n")
alphaTwoStageAlphaAlphaPre01N
print("alphaTwoStageAlphaAlphaPre01NBF\n")
alphaTwoStageAlphaAlphaPre01NBF
print("alphaTwoStageAlphaAlphaPreNoPreN\n")
alphaTwoStageAlphaAlphaPreNoPreN
print("alphaTwoStageAlphaAlphaPreNoPreNBF\n")
alphaTwoStageAlphaAlphaPreNoPreNBF
