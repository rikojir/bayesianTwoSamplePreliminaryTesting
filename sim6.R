...
sim = function(alpha){
  SWD1U = pValWilcoxD1U = rankSumBFD1U = matrix(1,nrow=s,ncol=length(n))
  for(i in 1:length(n)){
    for(j in 1:s){
      if(alpha != 1){ # pretest case
        while(SWD1U[j,i]>alpha){
          xDat=runif(n[i],0,1)
          yDat=runif(n[i],0,1)
          SWD1U[j,i]=min(shapiro.test(xDat)$p.value,shapiro.test(yDat)$p.value)
        }
      } else { # no pretest case
        xDat=runif(n[i],0,1)
        yDat=runif(n[i],0,1)
      }
    pValWilcoxD1U[j,i]=wilcox.test(xDat,yDat,conf.level = 0.95,paired=FALSE)$p.value
    rankSumBFD1U[j,i]=giveBayesFactorZeroOne(rankSumGibbsSampler(xVals=xDat,yVals=yDat,nSamples = 2500, progBar = FALSE))
    }
  }
  result = list(pValWilcoxD1U,rankSumBFD1U)
  result
}
...
# )
system.time(
  results6 <- mclapply(alpha_pre, sim, mc.cores = numCores)
)


# t-test p-values and JZS BFs
alphaWilcoxAlphaPre1U=alphaRankSumBFAlphaPre1U=numeric(length(n)) # p-values & BFs for alpha_pre = 0.100
alphaWilcoxAlphaPre05U=alphaRankSumBFAlphaPre05U=numeric(length(n)) # p-values & BFs for alpha_pre = 0.05
alphaWilcoxAlphaPre01U=alphaRankSumBFAlphaPre01U=numeric(length(n)) # p-values & BFs for alpha_pre = 0.01
alphaWilcoxAlphaPreNoPreU=alphaRankSumBFAlphaPreNoPreU=numeric(length(n)) # p-values & BFs for alpha_pre = 1 ( no pretest)

# Wilcox p-values and rankSumGibbsSampler BFs p-values
# alpha = 0.100
for(j in 1:length(n)){
      alphaWilcoxAlphaPre1U[j]=as.numeric(tbl_df(results6[[1]][[1]][,j]) %>% filter(value < 0.05) %>% summarise(length(value))/s)
}
for(j in 1:length(n)){
      alphaRankSumBFAlphaPre1U[j]=as.numeric(tbl_df(results6[[1]][[2]][,j]) %>% filter(value >= 3) %>% summarise(length(value))/s)
}

# alpha = 0.05
for(j in 1:length(n)){
      alphaWilcoxAlphaPre05U[j]=as.numeric(tbl_df(results6[[2]][[1]][,j]) %>% filter(value < 0.05) %>% summarise(length(value))/s)
}
for(j in 1:length(n)){
      alphaRankSumBFAlphaPre05U[j]=as.numeric(tbl_df(results6[[2]][[2]][,j]) %>% filter(value >= 3) %>% summarise(length(value))/s)
}

# alpha = 0.01
for(j in 1:length(n)){
      alphaWilcoxAlphaPre01U[j]=as.numeric(tbl_df(results6[[3]][[1]][,j]) %>% filter(value < 0.05) %>% summarise(length(value))/s)
}
for(j in 1:length(n)){
      alphaRankSumBFAlphaPre01U[j]=as.numeric(tbl_df(results6[[3]][[2]][,j]) %>% filter(value >= 3) %>% summarise(length(value))/s)
}

# no pretest
for(j in 1:length(n)){
      alphaWilcoxAlphaPreNoPreU[j]=as.numeric(tbl_df(results6[[4]][[1]][,j]) %>% filter(value < 0.05) %>% summarise(length(value))/s)
}
for(j in 1:length(n)){
      alphaRankSumBFAlphaPreNoPreU[j]=as.numeric(tbl_df(results6[[4]][[2]][,j]) %>% filter(value >= 3) %>% summarise(length(value))/s)
}

print("SIM6")
print("alphaWilcoxAlphaPre1U\n")
alphaWilcoxAlphaPre1U
print("alphaRankSumBFAlphaPre1U\n")
alphaRankSumBFAlphaPre1U
print("alphaWilcoxAlphaPre05U\n")
alphaWilcoxAlphaPre05U
print("alphaRankSumBFAlphaPre05U\n")
alphaRankSumBFAlphaPre05U
print("alphaWilcoxAlphaPre01U\n")
alphaWilcoxAlphaPre01U
print("alphaRankSumBFAlphaPre01U\n")
alphaRankSumBFAlphaPre01U
print("alphaWilcoxAlphaPreNoPreU\n")
alphaWilcoxAlphaPreNoPreU
print("alphaRankSumBFAlphaPreNoPreU\n")
alphaRankSumBFAlphaPreNoPreU
