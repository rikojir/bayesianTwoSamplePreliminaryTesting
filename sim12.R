  ...
  for(i in 1:length(n)){
    for(j in 1:s){
      if(alpha!=1){ # if alpha=1, no pretest, move on with t.test and JZS BF test
        xDat=runif(n[i],0.5,1.5) # differing parameters
        yDat=runif(n[i],0,1)
        ...
        if(SW1U[j,i]<alpha){
          ...
          BF1U[j,i]=giveBayesFactorZeroOne(rankSumGibbsSampler(xVals=xDat,yVals=yDat,nSamples = 2500,progBar = FALSE))
        }
      } else{
        xDat=runif(n[i],0.5,1.5) # differing parameters
        yDat=runif(n[i],0,1)
...
# )
system.time(
  betaResultsTwoStageProcedureU <- mclapply(alpha_pre, sim, mc.cores = numCores) # parallel execution for all alpha_pre levels
)

# beta levels
betaTwoStageAlphaPre1U=betaTwoStageAlphaPre1UBF=numeric(length(n)) # p-values & BFs for alpha_pre = 0.100
betaTwoStageAlphaAlphaPre05U=betaTwoStageAlphaAlphaPre05UBF=numeric(length(n)) # p-values & BFs for alpha_pre = 0.05
betaTwoStageAlphaAlphaPre01U=betaTwoStageAlphaAlphaPre01UBF=numeric(length(n)) # p-values & BFs for alpha_pre = 0.01
betaTwoStageAlphaAlphaPreNoPreU=betaTwoStageAlphaAlphaPreNoPreUBF=numeric(length(n)) # p-values & BFs for alpha_pre = 1 ( no pretest)

# alpha = 0.1
for(j in 1:length(n)){
  betaTwoStageAlphaPre1U[j]=as.numeric(tbl_df(betaResultsTwoStageProcedureU[[1]][[1]][,j]) %>% filter(value >= 0.05) %>% summarise(length(value))/s)
}
for(j in 1:length(n)){
  betaTwoStageAlphaPre1UBF[j]=as.numeric(tbl_df(betaResultsTwoStageProcedureU[[1]][[2]][,j]) %>% filter(value <= 3) %>% summarise(length(value))/s)
}

# alpha = 0.05
for(j in 1:length(n)){
  betaTwoStageAlphaAlphaPre05U[j]=as.numeric(tbl_df(betaResultsTwoStageProcedureU[[2]][[1]][,j]) %>% filter(value >= 0.05) %>% summarise(length(value))/s)
}
for(j in 1:length(n)){
  betaTwoStageAlphaAlphaPre05UBF[j]=as.numeric(tbl_df(betaResultsTwoStageProcedureU[[2]][[2]][,j]) %>% filter(value <= 3) %>% summarise(length(value))/s)
}

# alpha = 0.01
for(j in 1:length(n)){
  betaTwoStageAlphaAlphaPre01U[j]=as.numeric(tbl_df(betaResultsTwoStageProcedureU[[3]][[1]][,j]) %>% filter(value >= 0.05) %>% summarise(length(value))/s)
}
for(j in 1:length(n)){
  betaTwoStageAlphaAlphaPre01UBF[j]=as.numeric(tbl_df(betaResultsTwoStageProcedureU[[3]][[2]][,j]) %>% filter(value <= 3) %>% summarise(length(value))/s)
}

# no pretest
for(j in 1:length(n)){
  betaTwoStageAlphaAlphaPreNoPreU[j]=as.numeric(tbl_df(betaResultsTwoStageProcedureU[[4]][[1]][,j]) %>% filter(value >= 0.05) %>% summarise(length(value))/s)
}
for(j in 1:length(n)){
  betaTwoStageAlphaAlphaPreNoPreUBF[j]=as.numeric(tbl_df(betaResultsTwoStageProcedureU[[4]][[2]][,j]) %>% filter(value <= 3) %>% summarise(length(value))/s)
}

print("SIM12 - beta levels for uniform data")
print("betaTwoStageAlphaPre1U\n")
betaTwoStageAlphaPre1U
print("betaTwoStageAlphaPre1UBF\n")
betaTwoStageAlphaPre1UBF
print("betaTwoStageAlphaAlphaPre05U\n")
betaTwoStageAlphaAlphaPre05U
print("betaTwoStageAlphaAlphaPre05UBF\n")
betaTwoStageAlphaAlphaPre05UBF
print("betaTwoStageAlphaAlphaPre01U\n")
betaTwoStageAlphaAlphaPre01U
print("betaTwoStageAlphaAlphaPre01UBF\n")
betaTwoStageAlphaAlphaPre01UBF
print("betaTwoStageAlphaAlphaPreNoPreU\n")
betaTwoStageAlphaAlphaPreNoPreU
print("betaTwoStageAlphaAlphaPreNoPreUBF\n")
betaTwoStageAlphaAlphaPreNoPreUBF
